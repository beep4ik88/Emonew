...
async def get_entries_for_range(user_id: int, start_date: str, end_date: str):
    """Записи за диапазон дат включительно (ISO-строки YYYY-MM-DD, сравнение лексикографическое —
    работает для дат в этом формате). Используется для недельной сводки."""
    pool = await _get_pool()
    async with pool.acquire() as conn:
        rows = await conn.fetch(
            """SELECT primary_emotion, secondary_emotion, created_at, entry_date FROM entries
               WHERE user_id=$1 AND entry_date >= $2 AND entry_date <= $3 ORDER BY entry_date, created_at""",
            user_id, start_date, end_date,
        )
        return [
            (r["primary_emotion"], r["secondary_emotion"], r["created_at"], r["entry_date"])
            for r in rows
        ]
